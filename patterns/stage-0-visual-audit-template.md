...
> 任一新 product 起手都用：Climby / Coplot / adn studios / yen shine / 未來其他。**各跑一次，不要混合**。
> 預期執行時間：30–60 分鐘 per product。
>
> **2026-05-27 升格**：本檔原在 `<repo>/design/stage-0/audit-template.md`，現升格為跨 project 共用 vault asset。Coplot 自己的 audit run 仍留在 `<repo>/design/stage-0/audit-coplot.md`（frozen instance）。
...
這個產品已經在 dogfood 階段，有實際運行的 code base。你的工作是 **盤點目前 code 裡實際使用的視覺 token、component pattern、視覺一致性風險**，產出一份結構化報告，供後續的 brand token 落地（Stage 2）使用。
...
3. **Brand audit report**（缺口已標出）：
   - `{path-to-brand-audit.md}`（climby: `<climby-repo>/brand/audit.md` · coplot: `<repo>/brand/audit.md`）
...
1. **Brand audit 中的 G1-G4 reconciliation 已決策**
   - Coplot：以 `<repo>/brand/audit.md` §G1-G4 為清單
   - Climby：以對應 `<climby-repo>/brand/audit.md` 為清單
2. **Source-of-truth 文件已選定**
   - Coplot 預設：brandbook v3.0（per brand-audit §G1 推薦 Option A）
   - 衝突文件（如 OneBook Brief）的 conflicting items 必須已 patch 完
...
> Coplot：對照 brandbook v3.0（OneBook Brief 中的衝突項已視為已 patch）。
...
依產品的 brand book anti-pattern 清單，**明確 grep**。Coplot 範例（依 `<repo>/brand/brandbook.md` 與 CLAUDE.md R1-R10）：
...
> 📏 **Effort budget anchor**：Coplot Phase 1.5 W3-W4 約 **2 週（~10 工作天）** 給 design system 落地 + Phase 1 既有 screens 替換（per `OneBook_Roadmap.md` §3）。
> 建議的 migration order **必須 fit in 這個 budget**，超出的列為「Phase 1.5+1 backlog」，不要硬塞。

提出建議的 Stage 3 component 重皮順序：
...
輸出兩個 list：
- **Phase 1.5 W3-W4 in-scope**（順序排好，預估總工時 ≤ 10 d）
- **Phase 1.5+1 backlog**（超出 budget 的部分）
...
- [ ] Step 7 migration order fit 在 Phase 1.5 W3-W4 ~10 d budget 內，超出部分列入 backlog
...
| Variable | Climby value | Coplot value |
|---|---|---|
| `{PRODUCT_NAME}` | Climby | Coplot |
| `{path-to-brand-book.md}` | `<climby-repo>/brand/brandbook.md` | `<repo>/brand/brandbook.md` |
| `{path-to-designer-brief.md}` | `<climby-repo>/brand/designer-brief.md` | `<workspace>/archive/design-system-brief-v1.1.md` (archived) |
| `{path-to-brand-audit.md}` | `<climby-repo>/brand/audit.md` | `<repo>/brand/audit.md` |

---

*Template version 1.0 · 2026-05-18 · 給 Climby / Coplot Stage 0 共用*
